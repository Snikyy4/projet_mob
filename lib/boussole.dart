...
class _BoussoleState extends State<Boussole> {
  ...
  double _direction = 0;
  final double _randomDirection = Random().nextInt(360).toDouble(); // direction aléatoire à pointer
  bool _gameOver = false;
  final double _marginOfError = 10.0;
          ...
          if ((_direction - _randomDirection).abs() <= _marginOfError) {
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                ...
                content:
                    const Text('Fin du jeu ! Vous avez atteint la direction souhaitée'),
                actions: <Widget>[
                  TextButton(
                    child: const Text('OK'),
                    onPressed: () async {
